...
import com.web.demo.dao.DbSourceRepository;
...
public class DbService {
    ...
    static ConcurrentHashMap<String, String > dbType = new ConcurrentHashMap<String, String>();
    ...
    public R addDbSource(DbSourceModel config)throws GlobalException{
        try {
            if (config != null) {
                ...
                DBType.setDbType(config.getDriverClass().trim(), config.getDbname().trim());
                DbSourceModel save = dbSourceRepository.save(config);
                return save==null?R.error().message("保存数据源出错"):R.ok().message("保存数据源成功");
            }
        } catch (SQLException throwables) {
            log.error("SQL执行异常:{}",throwables);
            throw new GlobalException(ResultCodeEnum.UNKNOWN_ERROR.getCode(),"SQL执行异常");
        } catch (Exception e){
            log.error("保存数据源报错:{}",e);
            throw new GlobalException(ResultCodeEnum.UNKNOWN_ERROR.getCode(),"保存数据源报错");
        }
        return R.error().message("保存数据源出错");
    }

    public R testUrl(DbSourceModel model) {
        ...
        try {
            ...
            if (rs.first()) {
                return R.ok().message("连接成功！");
            } else {
                return R.error().message("连接失败,返回结果集为空！");
            }
        } catch (SQLException e) {
            log.error(e.getMessage());
            return R.error().message("连接失败！error:" + e.getMessage());
        } finally {
            try {
                if (rs != null) rs.close();
...
enum DBType {
    ...
    static void setDbType(String driverClassName,String sourceKey){
        String dbName="";
        ...
        DbService.dbType.put(sourceKey.trim(),dbName);
    }
